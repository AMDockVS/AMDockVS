"""The docking results table and the complex view built on it.

Rows come from `docking.results.repository` and the column layouts from
`docking.results.tables`; this module only renders and filters them. `ComplexWidget` is
the same table scoped to one complex — the pivots in `tools.docking.results_pivot` reuse it.
"""
...
from amdockvs.docking.results.tables import (
    fmt_float,
...
from amdockvs.ui.catalog.common import project_table
from ms_components.ms_table import FilterOperator, FilterSpec, MetricFilterBar, SmartTableView
...
class DockingResultsWidget(QWidget):
    ...
    def _load_interactions(self, hit: DockingHitSummary) -> None:
        ...
        from amdockvs.docking.results.diagram import pose_interactions
    ...
    def _build_diagram(self) -> None:
        ...
        def work():
            from amdockvs.docking.results.diagram import build_pose_diagram, save_pose_diagram
