...
class BindingSitesWidget(QWidget):
    ...
    def refresh(self) -> None:
        ...
        if receptor_id is None:
            ...
            return
        run_async(
            lambda: self.runtime.binding_sites.list_predictions(receptor_id=receptor_id),
    ...
    def _show_sites_in_pymol(self, sites: list[BindingSite]) -> None:
        ...
        try:
            receptor = self.runtime.binding_sites.get_receptor(int(focus.molecule_id))
    ...
    def _activate_selected(self) -> None:
        ...
        if site is None:
            return
        try:
            self.runtime.binding_sites.set_active_site(
                molecule_id=int(site.molecule_id),
                binding_site_id=int(site.id or 0),
            )
            receptor = self.runtime.binding_sites.get_receptor(int(site.molecule_id))
...
class PocketDetectionWidget(QWidget):
    ...
    def refresh(self) -> None:
        run_async(
            lambda: (self.runtime.binding_sites.tool_status(), self.runtime.binding_sites.list_receptors()),
            self._apply_refresh,
    ...
    def _run_prediction(self) -> None:
        ...
        if not self._tool_ready:
            tool = self.runtime.binding_sites.tool_status()
            QMessageBox.information(
                self,
    ...
    def _submit_prediction(self) -> None:
        ...
        try:
            plan = self.runtime.binding_sites.plan_prediction(
            ...
            if not plan.recalculate_receptor_ids:
                ...
                return
            job_id = self.runtime.binding_sites.predict(
                receptor_ids=receptor_ids,
                profiles=profiles,
