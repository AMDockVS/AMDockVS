...
def build_pose_diagram(
    *,
    ...
    """
    from amdockvs.docking.results.interactions import (
        _rdkit_mol,
        _write_ligand_pdb,
        as_pdb_block,
        atom_count,
...
def render_diagram_subprocess(
    *,
    ...
    try:
        proc = subprocess.run(
            [sys.executable, "-m", "amdockvs.docking.results.diagram", json.dumps(spec)],
            capture_output=True,
            text=True,
            timeout=timeout,
            env=env,
...
def _selfcheck() -> None:
    ...
    from amdockvs.docking.results.interactions import as_pdb_block, atom_count
